...
void Type::emitMethod(const Emitter &e, primitives::CppEmitter &h, primitives::CppEmitter &cpp) const
{
    ...
    auto get_parameters = [this, &has_input_file](bool defaults, int last_non_optional = -1)
    {
        ...
        if (!fields.empty())
            ctx.trimEnd(1);
        return std::tuple{ ctx.getText(),last_non_optional };
    };
    auto [params,lno] = get_parameters(false);

    auto emit_decl = [this, &h, &get_parameters, &lno](bool async) {
        // usual call
        ...
        {
            h.increaseIndent();
            h.addLine(std::get<0>(get_parameters(true, lno)));
            h.decreaseIndent();
            h.addLine();
        }
        h.addText(") const;");
        ...
        if (!fields.empty())
        {
            ...
            if (async)
                h.addText("Async");
            h.addText("(const " + name + "Request &) const;");
            h.emptyLines();
        }
    };
    emit_decl(false);
    ...
    auto emit_def = [this, &cpp, &params, &has_input_file](bool async) {
        // usual call
        cpp.addLine();
        if (async)
            cpp.addText("api::awaitable<");
        ...
        if (has_input_file)
        {
            ...
            cpp.addLine("auto j = SEND_REQUEST");
            if (async)
                cpp.addText("_ASYNC");
            cpp.addText("(" + name + ", args);");
        }
        else
        {
            ...
            cpp.addLine("j = SEND_REQUEST");
            if (async)
                cpp.addText("_ASYNC");
            cpp.addText("(" + name + ", j.dump());");
        }
        cpp.addLine();
        if (async)
            cpp.addText("co_");
        ...
        if (!fields.empty())
        {
            ...
            if (async)
                cpp.addText("Async");
            cpp.addText("(const " + name + "Request &r) const");
            cpp.beginBlock();
            cpp.addLine();
            cpp.addText("return " + name);
            if (async)
                cpp.addText("Async");
            cpp.addText("(");
            cpp.increaseIndent();
